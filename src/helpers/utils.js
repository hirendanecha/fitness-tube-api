...
exports.notificationMail = async (userData) => {
  ...
  const mailObj = {
    email: userData.email,
    subject: "Fitness trainer notification",
...
exports.channelNotificationEmail = async (userData) => {
  ...
  let msg = `You have been assign in FitnessTube channel by the FitnessTube Admin.
  To access your channel, log into your FitnessTube account,click on the
  FitnessTube icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `FitnessTube has approved your Trainer application.`;
    } else {
      msg = `FitnessTube has unapproved your Trainer application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
