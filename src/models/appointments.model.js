...
var Appointments = function (appointment) {
  ...
  this.trainerProfileId = appointment.trainerProfileId;
  this.trainerName = appointment.trainerName;
...
Appointments.findAndSearchAll = async (
  ...
  let whereCondition = `${search ? `a.trainerName LIKE '%${search}%'` : ""}`;
  ...
  const searchData = await executeQuery(
    `SELECT a.id, a.trainerName, a.appointmentDateTime, a.createdDate,a.profileId,a.isCancelled,p.FirstName,p.LastName,p.ProfilePicName,p.Username FROM appointments as a left join profile as p on p.ID = a.profileId  ${
      whereCondition ? `WHERE ${whereCondition} ` : ""
